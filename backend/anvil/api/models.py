...
async def common_phases_across_models(
    ...
    NOTE: This handler and `/compare` below MUST stay declared BEFORE
    `/{slug}` so FastAPI does not route `/compare` and
    `/compare/common-phases` into the single-slug model detail handler.
    Moving them below `/{slug}` causes `GET /api/models/compare` to be
    interpreted as `{slug=compare}` and return 404 "Model not found".
    """
